...
import { lookupAres } from "@/app/(app)/vydane-doklady/invoice-actions";
import { saveQuote } from "@/app/(app)/nabidky/actions";
...
export function QuoteForm({
  ...
  quote?: Tables<"quotes">;
  existingItems?: Tables<"quote_line_items">[];
  ...
  const [icoInput, setIcoInput] = useState(quote?.customer_ico ?? preselected?.ico ?? "");
  const [customerName, setCustomerName] = useState(quote?.customer_name ?? preselected?.name ?? "");
  const [customerAddress, setCustomerAddress] = useState(
    quote?.customer_address ?? preselected?.address ?? ""
  );
  const [customerDic, setCustomerDic] = useState(quote?.customer_dic ?? preselected?.dic ?? "");
  ...
  const quoteNumber = quote?.quote_number ?? suggestedNumber;

  return (
    <form action={saveQuote} className="max-w-4xl space-y-6">
      ...
      <div className="rounded-md border border-slate-200 bg-slate-50 px-4 py-2 text-sm text-slate-600">
        Číslo nabídky: <span className="font-semibold text-[#1e3a5f]">{quoteNumber}</span>
      </div>

      {/* ZÁKAZNÍK */}
      <div>
        <h2 className="text-sm font-semibold text-slate-700 mb-2">Zákazník</h2>

        {existingPartners.length > 0 ? (
          <label className="block mb-3 max-w-sm">
            <span className="block text-xs font-medium text-slate-500 mb-1">
              Vybrat uloženého zákazníka
            </span>
            <select
        ...
        <div className="flex items-end gap-2 mb-3">
          <label className="text-xs flex-1 max-w-xs">
            <span className="block text-slate-500 mb-1">IČO (nepovinné)</span>
            <input
              value={icoInput}
      ...
      <div className="grid grid-cols-3 gap-4">
        {field(
          "Datum vystavení",
          <input type="date" name="issue_date" defaultValue={quote?.issue_date ?? todayISO()} className={inputClass} />
        )}
        {field(
          "Platnost nabídky do",
          <input
            type="date"
            name="valid_until"
            defaultValue={quote?.valid_until ?? addDaysISO(todayISO(), 30)}
            className={inputClass}
          />
        )}
        {field(
          "Stav",
          <select name="status" defaultValue={quote?.status ?? "navrh"} className={inputClass}>
            <option value="navrh">Návrh</option>
            <option value="odeslana">Odeslána</option>
            <option value="prijata">Přijata zákazníkem</option>
            <option value="zamitnuta">Zamítnuta</option>
          </select>
        )}
      </div>
      ...
      <div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-xs text-slate-400 border-b border-slate-100">
                ...
                <th className="px-3 py-2 font-medium w-32">Celkem</th>
        ...
        <button type="button" onClick={addItem} className="mt-2 text-xs text-[#1e3a5f] hover:underline">
          + Přidat položku
        </button>
      </div>
      ...
      {field("Poznámka", <textarea name="note" defaultValue={quote?.note ?? ""} rows={2} className={inputClass} />)}
